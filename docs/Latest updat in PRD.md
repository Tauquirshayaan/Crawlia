# Clearpitch — Website Analysis & Outreach Automation Platform
...
A note on working name: this document uses **Clearpitch** as the working product name throughout so nothing reads like a placeholder. A full naming shortlist with rationale is provided separately at the end of the accompanying response — swap the name via find-and-replace once one is chosen; nothing in the architecture depends on it.

---

## 1. Document control & purpose

This PRD defines the complete v1 (and near-term v2) scope for Clearpitch, a SaaS platform that finds businesses, audits their live websites the way a senior designer, SEO consultant, and performance engineer would, and turns the specific problems it finds into individually personalized cold outreach emails — then sends, tracks, and triages the replies. It is written to remove ambiguity for whoever builds it: every functional requirement includes inputs, outputs, states, and edge-case handling, not just a feature name.
...
| Product | Core angle | Where it's strong | Gap Clearpitch should exploit |
|---|---|---|---|
...
- Public API + webhooks for agencies wanting to embed Clearpitch in their own tooling
...
- **Segment/vertical tagging:** every lead carries a `segment` field (auto-filled from the lead-gen search category, or a required column on CSV/Sheet import) — e.g. "law firms," "hospitality," "B2B SaaS." Campaigns are built from one segment at a time by default, since a redesign pitch to a law firm and a pitch to a restaurant should not share the same framing; mixing segments into one campaign is allowed but surfaced with a warning, not silently permitted.
...
**Acceptance criteria:** a parked domain, a domain returning only a CAPTCHA wall, and a domain timing out all reach the Rules engine with zero credits charged and a clear, distinct reason code surfaced to the user (not a generic "failed").
...
- **Step A — finding-to-angle mapping (explicit, inspectable, not folded into the final draft silently):** the 1–3 most severe findings from Section 8.3 Stage 4 are first translated into short messaging angles before any final prose is written — e.g. a raw finding of "LCP 4.8s on mobile" becomes an angle like "first impression / drop-off risk," a raw finding of "cluttered nav hierarchy" becomes "user friction." This mapping is stored as its own field on the draft record (not just embedded inside the final email text), so a reviewer or a later analytics pass can see which angle was used without re-reading prose, and so the angle vocabulary can be tuned centrally (Section 13) without re-touching the final-compose prompt. This is a deliberate two-step design rather than one opaque pass — it costs one extra small model call per lead but makes tone and claim consistency auditable at scale, which a single end-to-end call cannot give you.
- **Step B — compose:** lead fields (name, company, city, industry, any custom CSV column) and campaign-level writing settings (language, tone, signature block, closing CTA — e.g. "offer a free mockup," configurable per campaign) are combined with the Step A angles into subject + body, produced together in one call (so the subject reflects what the body actually says, not a mismatched mail-merge line), returned as validated JSON `{ subject, body }`.
...
- **Prioritized review order:** the review queue defaults to lowest-score-first (the worst-scoring, highest-opportunity sites), not import order — reviewing selectively by value beats reading a campaign top-to-bottom, and the highest-value/lowest-scoring leads are exactly the ones worth a human's attention before the rest get batch-approved.
...
- **Workspace level:** total leads, sites analyzed, emails sent, aggregate open/click/reply rates, credits remaining vs. used this billing period.
...
**Acceptance criteria:** every number shown on the dashboard must be reproducible from the underlying event tables via a documented query — no dashboard metric may exist only as a cached/derived value with no auditable source.
...
- **Prompt-injection guardrail (adversarial page content):** the extracted DOM/text and screenshot come from a third party's website, not from the platform's own user — they must be treated strictly as untrusted data inside the prompt, never as instructions. A hostile site can embed hidden text (white-on-white, off-screen positioning, stuffed into `alt` attributes) instructing the model to award a perfect score, ignore its rubric, or output attacker-chosen text. Mitigations: wrap extracted content in clearly delimited data blocks with an explicit system instruction to treat anything imperative inside them as content to critique, never as a command to follow; strip common injection phrasing before it reaches the model; cross-check the returned score against the objective Stage 3 signals as a plausibility check (a site with failing Core Web Vitals and no meta description returning a perfect 10 is itself a signal something was manipulated); and ensure the writing engine (Section 8.5) only ever forwards the model's own structured findings into an outbound email, never raw scraped page text verbatim — this bounds how much attacker-controlled content could ever reach a real inbox.
...
Design system: clean, low-chrome SaaS aesthetic; the primary user is often reviewing dozens of AI-generated drafts in a row, so information density and fast keyboard-driven review (approve/edit/regenerate without a page reload) matter more than decorative polish. Responsive down to tablet width is sufficient — this is a desktop-first, agency-user product, not a mobile-native one.

Key screens (each needs its own detailed wireframe pass before build, not specified pixel-by-pixel here):
- **Onboarding:** connect a mailbox or skip, import or generate a first small lead batch, run one real analysis before asking for a credit card.
- **Dashboard:** the Section 8.9 metrics, recent replies, a clear next action ("3 drafts waiting for review").
- **Leads list + detail:** filterable/searchable table (Section 8.2) and a single-lead deep-dive view showing the full audit.
...
- **Inbox:** unified thread view with intent-classification tags (Section 8.8) visually prominent.
